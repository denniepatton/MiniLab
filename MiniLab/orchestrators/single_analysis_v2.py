...
async def _build_script_with_tools(
    ...
    """
    Build a script using TRUE incremental editing tools (like a VS Code agent).
    
    Instead of asking for whole chunks in prompts, Hinton uses tools to:
    1. Create an empty file with skeleton
    2. Append imports (and check syntax)
    3. Append functions one at a time (and check syntax)
    4. Append main block
    5. Run and see actual output
    6. Make SURGICAL fixes to specific lines based on errors
    
    This uses FEWER TOKENS because:
    - We don't re-send the whole file every time
    - Fixes are surgical (change line 45) not regenerative
    ...
    print(f"      🔨 Building script with incremental tools...")
    
    # STEP 1: Create the file with basic skeleton
    print(f"        Creating {script_name}...")
    skeleton = f'''"""
{script_name} - Generated by MiniLab
{script_spec[:200]}...
"""
...
# Paths
DATA_DIR = Path("ReadData")
OUTPUT_DIR = Path("Sandbox/{project_name}/{output_dir_name}")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
'''
    
    result = await hinton.use_tool("code_editor", action="create", 
                                    path=script_rel_path, skeleton=skeleton)
    if not result.get("success"):
        # File might already exist - try to view it
        result = await hinton.use_tool("code_editor", action="view", path=script_rel_path)
    
    tokens_used += 100  # Tool overhead
    
    # STEP 2: Ask Hinton what additional imports are needed
    print(f"        Adding imports...")
    import_prompt = f"""Looking at this task, what additional imports do I need?

TASK: {script_spec[:1200]}

{pkg_constraints}

I already have: numpy, pandas, pathlib.Path

List ONLY the additional import statements needed (one per line), like:
import matplotlib.pyplot as plt
from scipy import stats
etc.

Be minimal - only imports actually needed for this task."""

    import_response = await hinton.arespond(import_prompt, max_tokens=500)
    tokens_used += TOKEN_PER_CALL
    
    # Extract imports and append them
    import_lines = []
    ...
    if import_lines:
        import_code = '\n'.join(import_lines)
        # Insert after existing imports (around line 10)
        await hinton.use_tool("code_editor", action="insert", 
                             path=script_rel_path, line=9, code=import_code)
        tokens_used += 100
    
    # Check syntax
    syntax_result = await hinton.use_tool("code_editor", action="check_syntax", path=script_rel_path)
    if not syntax_result.get("syntax_valid", False):
        print(f"          ⚠️ Syntax error after imports: {syntax_result.get('error_message', '')}")
        # View the problematic area and fix
        view_result = await hinton.use_tool("code_editor", action="view", path=script_rel_path)
        fix_prompt = f"""Fix this syntax error:

{syntax_result.get('error_message')} at line {syntax_result.get('error_line')}

Current file:
{view_result.get('content', '')[:1500]}

What specific line(s) need to change? Respond with the line number and corrected code."""
        fix_response = await hinton.arespond(fix_prompt, max_tokens=500)
        tokens_used += TOKEN_PER_CALL
        # TODO: parse and apply fix
    
    print(f"          ✓ Imports ready")
    
    # STEP 3: Ask Hinton to write functions ONE AT A TIME
    print(f"        Writing functions...")
    
    functions_prompt = f"""For this task, what functions do I need to write?

TASK: {script_spec[:1200]}

List each function with a brief description:
1. function_name - what it does
2. another_function - what it does
etc.

Just the list, I'll write them one at a time."""

    func_list_response = await hinton.arespond(functions_prompt, max_tokens=500)
    tokens_used += TOKEN_PER_CALL
    
    # Parse function list (rough extraction)
    import re
    func_matches = re.findall(r'\d+\.\s*(\w+)\s*[-–—:]\s*(.+)', func_list_response)
    if not func_matches:
        # Fallback: just ask for the main logic
        func_matches = [("process_data", "main processing logic")]
    
    for func_name, func_desc in func_matches[:5]:  # Limit to 5 functions
        print(f"          Writing {func_name}()...")
        
        func_prompt = f"""Write the function `{func_name}` for this task.

Purpose: {func_desc}
Full task context: {script_spec[:800]}

Write ONLY this one function with docstring. No imports, no main block.
Use print() statements for progress. Include error handling."""

        func_response = await hinton.arespond(func_prompt, max_tokens=2000)
        tokens_used += TOKEN_PER_CALL
        
        func_code = extract_code_from_response(func_response)
        if func_code:
            # Clean: remove any stray imports or main blocks
            func_lines = []
            ...
            if func_code.strip():
                await hinton.use_tool("code_editor", action="append",
                                     path=script_rel_path, code=func_code)
                tokens_used += 100
                
                # Quick syntax check
                syntax_result = await hinton.use_tool("code_editor", action="check_syntax", 
                                                      path=script_rel_path)
                if not syntax_result.get("syntax_valid", False):
                    print(f"            ⚠️ Syntax error in {func_name}, fixing...")
                    # Get context around error
                    error_line = syntax_result.get("error_line", 1)
                    view_result = await hinton.use_tool("code_editor", action="view",
                                                        path=script_rel_path,
                                                        start_line=max(1, error_line-5),
                                                        end_line=error_line+5)
                    fix_prompt = f"""Syntax error at line {error_line}: {syntax_result.get('error_message')}

{view_result.get('content', '')}

What is the corrected line {error_line}? Provide ONLY the fixed line."""
                    fix_response = await hinton.arespond(fix_prompt, max_tokens=200)
                    tokens_used += TOKEN_PER_CALL
                    
                    fixed_line = fix_response.strip()
                    if fixed_line and not fixed_line.startswith('#'):
                        await hinton.use_tool("code_editor", action="replace",
                                             path=script_rel_path,
                                             start_line=error_line, end_line=error_line,
                                             code=fixed_line)
                        tokens_used += 100
    
    print(f"          ✓ Functions written")
    
    # STEP 4: Write main block
    print(f"        Writing main block...")
    
    # View current state
    view_result = await hinton.use_tool("code_editor", action="view", path=script_rel_path)
    current_content = view_result.get("content", "")
    
    main_prompt = f"""Write the main block for this script.

Current file (summarized):
{current_content[:2000]}

TASK: {script_spec[:500]}

Write the `if __name__ == "__main__":` block that:
1. Loads data from ReadData/
2. Calls the functions defined above
3. Saves outputs to Sandbox/{project_name}/{output_dir_name}/
4. Ends with print("SCRIPT COMPLETE: [list of files created]")

Provide ONLY the main block, nothing else."""

    main_response = await hinton.arespond(main_prompt, max_tokens=1500)
    tokens_used += TOKEN_PER_CALL
    
    main_code = extract_code_from_response(main_response)
    if main_code:
        # Ensure it has the guard
        if 'if __name__' not in main_code:
            main_code = 'if __name__ == "__main__":\n    ' + main_code.replace('\n', '\n    ')
        
        await hinton.use_tool("code_editor", action="append",
                             path=script_rel_path, code=main_code)
        tokens_used += 100
    
    # STEP 5: Run and iterate on errors
    print(f"        Running script...")
    
    for attempt in range(1, MAX_FIX_ATTEMPTS_PER_SCRIPT + 1):
        run_result = await hinton.use_tool("code_editor", action="run", path=script_rel_path)
        tokens_used += 100
        ...
        if run_result.get("returncode") == 0:
            ...
            # Reconstruct code from view (strip line numbers)
            lines = []
            ...
            error_line = run_result.get("error_line")
            ...
            # Get API documentation for the error
            view_result = await hinton.use_tool("code_editor", action="view", path=script_rel_path)
            current_code = view_result.get("content", "")
            api_docs = get_documentation_for_code(current_code, max_chars=1500)
            
            # Ask for a SURGICAL fix
            if error_line:
                # View around the error
                context_result = await hinton.use_tool("code_editor", action="view",
                                                       path=script_rel_path,
                                                       start_line=max(1, error_line-10),
                                                       end_line=error_line+10)
                fix_prompt = f"""Script error:
{stderr[:1000]}

Code around line {error_line}:
{context_result.get('content', '')}

{api_docs if api_docs else ""}

What specific line(s) need to change? Format:
...
What specific changes fix this? Format:
...
[new code]"""
            ...
            line_fixes = re.findall(r'LINE\s*(\d+):\s*(.+)', fix_response, re.IGNORECASE)
            for line_num, new_code in line_fixes:
                await hinton.use_tool("code_editor", action="replace",
                                     path=script_rel_path,
                                     start_line=int(line_num), end_line=int(line_num),
                                     code=new_code.strip())
                tokens_used += 100
            
            # Also check for REPLACE LINES pattern
            replace_match = re.search(
                r'REPLACE LINES?\s*(\d+)-(\d+)\s*WITH:\s*```(?:python)?\s*(.*?)```',
                fix_response, re.IGNORECASE | re.DOTALL
            )
            if replace_match:
                start, end, code = replace_match.groups()
                await hinton.use_tool("code_editor", action="replace",
                                     path=script_rel_path,
    ...
    print(f"        ⚠️ Failed after {MAX_FIX_ATTEMPTS_PER_SCRIPT} attempts")
