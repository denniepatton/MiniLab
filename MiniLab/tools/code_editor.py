...
class TerminalTool(Tool):
    ...
    - Check environment
    - Run tests
    - Install packages (with restrictions)
    """
    ...
    def __init__(
        ...
        super().__init__(
            name="terminal",
            description="Run shell commands in the workspace. Use for running scripts, checking files, etc."
        )
        self.workspace_root = Path(workspace_root).resolve()
        # Whitelist of command prefixes
        self.allowed_commands = allowed_commands or [
            "python", "micromamba", "pip", "ls", "cat", "head", "tail",
    ...
    def _is_command_allowed(self, command: str) -> bool:
        ...
        return any(cmd_start.startswith(allowed) for allowed in self.allowed_commands)
    ...
    async def execute(self, action: str, **kwargs) -> Dict[str, Any]:
        ...
        Actions:
        - run: Run a shell command (requires: command, optional: timeout)
        """
        if action == "run":
            ...
            if not self._is_command_allowed(command):
                return {
                    "success": False,
                    "error": f"Command not allowed. Allowed prefixes: {self.allowed_commands}",
                }
